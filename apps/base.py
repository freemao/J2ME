...
J2MEHELP = "plant phenomics machine learning library v%s [%s]\n" % (__version__, __copyright__)
...
class OptionParser(OptionP):
    def __init__(self, doc):
        OptionP.__init__(self, doc, epilog=J2MEHELP)
...
def _find_ignore(mdir):
    ignores = []
    ignore_fn = Path(mdir)/'.ignore'
    if ignore_fn.exists():
        with open(ignore_fn) as f:
           for i in f:
                if i.startswith('#'):
                    continue
                ignores.append(i.strip())
    return ignores

def dmain(mainfile, type='action'):
    cwd = op.dirname(mainfile)
    ignore_scripts = _find_ignore(cwd)
    pyscripts = [x for x in glob(op.join(cwd, "*", '__main__.py'))] \
        if type == "module" \
        else [x for x in glob(op.join(cwd, "*.py")) if Path(x).name not in ignore_scripts]
    actions = []
